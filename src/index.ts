...
export type PredictiveSearchResourceType =
    | "product"
    | "page"
    | "article"
    | "collection"
...
export class Siopa {
    ...
    */
    ...
    private _emit<K extends keyof ShopifyEventMap>(
        event: K,
        data: ShopifyEventMap[K]
    ) {
    ...
    */
    ...
    async removeLineItems(payload: { ids: string[] }) {
        const idsMap = payload.ids.reduce((acc, id) => {
            acc[id] = 0;
            return acc;
